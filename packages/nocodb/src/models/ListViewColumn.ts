...
export default class ListViewColumn {
  ...
  static async list(
    ...
    const cached = await NocoCache.getList(CacheScope.LIST_VIEW_COLUMN, [
      viewId,
    ]);
    let { list } = cached;
    if (!cached.isNoneList && !list.length) {
      ...
      await NocoCache.setList(CacheScope.LIST_VIEW_COLUMN, [viewId], list);
    }
    list.sort(
      (a, b) =>
    ...
    return list.map((column) => new ListViewColumn(column));
  }
  ...
  static async get(context: NcContext, id: string, ncMeta = Noco.ncMeta) {
    ...
    if (!column) {
      ...
      if (column) {
        await NocoCache.set(`${CacheScope.LIST_VIEW_COLUMN}:${id}`, column);
      }
    }
  ...
  static async insert(
    ...
    insertObj.order =
      column.order ??
      (await ncMeta.metaGetNextOrder(MetaTable.LIST_VIEW_COLUMNS, {
    ...
    await View.clearSingleQueryCache(
      context,
      (
        await View.get(context, column.fk_view_id, ncMeta)
      ).fk_model_id,
      undefined,
      ncMeta,
    );
  ...
  static async update(
    ...
    await NocoCache.update(`${CacheScope.LIST_VIEW_COLUMN}:${id}`, updateObj);
    const column = await this.get(context, id, ncMeta);
