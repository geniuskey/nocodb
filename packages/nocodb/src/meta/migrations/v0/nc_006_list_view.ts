...
const up = async (knex: Knex) => {
  await knex.schema.createTable(MetaTable.LIST_VIEW, (table) => {
    ...
    table.string('fk_subtitle_column_id', 20);
    table.string('fk_image_column_id', 20);
    table.string('density', 20).notNullable().defaultTo('comfortable');
    table.boolean('show_field_labels').notNullable().defaultTo(true);
    table.text('meta');
    ...
    table.index(
      ...
      'nc_list_view_v2_base_workspace_idx',
    );
    table.index(['fk_view_id'], 'nc_list_view_v2_view_idx');
  });
  ...
  await knex.schema.createTable(MetaTable.LIST_VIEW_COLUMNS, (table) => {
    ...
    table.index(
      ['base_id', 'fk_workspace_id'],
      'nc_list_view_columns_v2_base_workspace_idx',
    );
    table.index(
      ['fk_view_id', 'fk_column_id'],
      'nc_list_view_columns_v2_view_column_idx',
    );
    table.index(['fk_view_id'], 'nc_list_view_columns_v2_view_idx');
  });
};
