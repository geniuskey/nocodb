...
Record and structural Trash expiry cleanup is scheduled automatically at minute
15 of every hour. For maintenance only, set
`NC_RECORD_TRASH_CLEANUP_DISABLED=true` before starting the backend to suspend
permanent removal of expired records, views, fields, and tables. See
[TRASH_RESTORE.md](./TRASH_RESTORE.md) for retention and batch limits.

Base Snapshot capture locks expire after 360 minutes by default. For recovery
testing or unusually large Bases, set `NC_SNAPSHOT_CAPTURE_TIMEOUT_MINUTES` to
an integer from 5 through 1440 before starting the backend. See
...
- Append-only metadata migration ledger: 117 files and 114 ordered registrations hash-verified across the v0, v1, v2, and audit tracks.
