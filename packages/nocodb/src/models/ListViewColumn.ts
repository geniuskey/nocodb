import type { BoolType, ListColumnType } from 'nocodb-sdk';
...
export default class ListViewColumn implements ListColumnType {
  id: string;
  show: BoolType;
  order: number;
  fk_view_id: string;
  ...
  public static async get(
    ...
    let viewColumn =
      ...
      (await NocoCache.get(
        context,
        `${CacheScope.LIST_VIEW_COLUMN}:${listViewColumnId}`,
    ...
    return viewColumn && new ListViewColumn(viewColumn);
  }
  ...
  public static async list(
    ...
    return columns.map((column) => new ListViewColumn(column));
  }
  ...
  static async update(
    ...
    const listColumn = await this.get(context, columnId, ncMeta);
    const view = await View.get(context, listColumn.fk_view_id, ncMeta);
    await View.clearSingleQueryCache(context, view.fk_model_id, [view], ncMeta);
