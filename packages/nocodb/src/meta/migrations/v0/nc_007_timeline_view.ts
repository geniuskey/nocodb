...
const up = async (knex: Knex) => {
  await knex.schema.createTable(MetaTable.TIMELINE_VIEW, (table) => {
    ...
    table.index(
      ...
      'nc_timeline_view_v2_base_workspace_idx',
    );
    table.index(['fk_view_id'], 'nc_timeline_view_v2_view_idx');
  });
  ...
  await knex.schema.createTable(MetaTable.TIMELINE_VIEW_COLUMNS, (table) => {
    ...
    table.index(
      ['base_id', 'fk_workspace_id'],
      'nc_timeline_view_columns_v2_base_workspace_idx',
    );
    table.index(
      ['fk_view_id', 'fk_column_id'],
      'nc_timeline_view_columns_v2_view_column_idx',
    );
    table.index(['fk_view_id'], 'nc_timeline_view_columns_v2_view_idx');
  });
};
